var RepeatingGameObject = function() {
	this.width = 0;
	this.height = 0;
	this.scrollFactor = 1;
	
	this.startupRepeatingGameObject = function(game, image, x, y, z, width, height, scrollFactor) {
	...
	this.shutdownRepeatingGameObject = function() {
	...
	this.draw = function(canvas, scroll) {
		...
		for (var y = 0; y < this.height; y += areaDrawn[1]) {
			for (var x = 0; x < this.width; x += areaDrawn[0]) {
				...
				var newPosition = [this.x[0] + x, this.x[1] + y];
				...
				if (x==0) newScrollPosition[0] = parseInt(scroll[0] * this.scrollFactor);
				if (y==0) newScrollPosition[1] = parseInt(/*scroll[1]*/ 0 * this.scrollFactor);
	...
	this.drawRepeat = function(canvas, newPosition, newFillArea, newScrollPosition) {
		...
		canvas.drawImage(this.image, parseInt(left), parseInt(top), parseInt(width), parseInt(height), parseInt(newPosition[0]), parseInt(newPosition[1]), parseInt(width), parseInt(height));
		return [width, height];
	};	
};
RepeatingGameObject.prototype = new GameObject();